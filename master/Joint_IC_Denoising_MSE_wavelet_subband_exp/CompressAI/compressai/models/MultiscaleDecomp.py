...
from .wavelet_subband_snr import SubbandSNRNet
from . import utils
import functools
import warnings
...
class MultiscaleDecomp(Cheng2020Anchor):
    ...
    '''
    for multiscale-decomp_mse_q1~multiscale-decomp_mse_q6
    '''
    ...
    def _enhance_non_local_feature(self, non_local_feature, mask, transformer):
        h_feature = non_local_feature.shape[2]
        w_feature = non_local_feature.shape[3]
        mask_resized = F.interpolate(mask, size=[h_feature, w_feature], mode='nearest')

        non_local_feature_unfold = F.unfold(
            non_local_feature, kernel_size=4, dilation=1, stride=4, padding=0
        )
        non_local_feature_unfold = non_local_feature_unfold.permute(0, 2, 1)

        mask_unfold = F.unfold(mask_resized, kernel_size=4, dilation=1, stride=4, padding=0)
        ...
        non_local_feature_unfold = transformer(
            non_local_feature_unfold, None, src_mask=mask_unfold
        )
        non_local_feature_unfold = non_local_feature_unfold.permute(0, 2, 1)
        non_local_feature_fold = nn.Fold(
            output_size=(h_feature, w_feature),
            kernel_size=(4, 4),
            stride=4,
            padding=0,
            dilation=1
        )(non_local_feature_unfold)

        return non_local_feature_fold, mask_resized

    def _blend_local_non_local(self, local_feature, non_local_feature, mask):
        channel = non_local_feature.shape[1]
    ...
    def g_a_func(self, x, x_blur=None, denoise=False):
        ...
        if denoise:
            if self.use_subband_snr:
                subband_masks = self.subband_snr_net(x)
            elif x_blur != None:
                mask = self.get_snr(x, x_blur)
            else:
                raise ValueError("x_blur is required when use_subband_snr is False")
        ...
        if denoise:
            ...
            if self.use_subband_snr:
                ...
                high_frequency_feature = self.high_frequency_fusion(
                    torch.cat(hf_features, dim=1)
                )
    ...
    '''
    General parts, no need to change
    '''
    def forward(self, x, x_blur=None, gt=None):
        # g_a for noisy input
        y_inter, y, subband_masks = self.g_a_func(x, x_blur=x_blur, denoise=True)
    ...
    def compress(self, x, x_blur=None):
        ...
        _, y, _ = self.g_a_func(x, x_blur=x_blur, denoise=True)
        z = self.h_a(y)
