...
if update and opt['path'].get('checkpoint', None):
    ...
    checkpoint = torch.load(opt['path']['checkpoint'], map_location=device_id)
...
for device, estimation, test_loader in runs:    
    ...
    for i, data in enumerate(test_loader):
        ...
        if len(data) == 2:
            ...
            img_nf = img_nf.to(device)
            noise = util.cropping(noise)
            img_nf = util.cropping(img_nf)
        else:
            gt, noise, img_nf = data
            gt = gt.to(device)
            gt = util.cropping(gt)
            noise = noise.to(device)
            noise = util.cropping(noise)
        ...
        if estimation:
            start = time.time()
            out_net = model.forward(noise, x_blur=img_nf, gt=gt)
        ...
        # coder mode using model.compress() and model.decompress()
            ...
            start = time.time()
            out_enc = model.compress(noise, x_blur=img_nf)
        ...
        msssimDB = -10 * (np.log(1 - cur_ssim) / np.log(10))
        denoise = util.torch2img(rec[0])
        denoise.save(os.path.join(save_dir, '{:04d}_{:.3f}dB_{:.4f}_{:.4f}dB_{:.4f}bpp.png'.format(i, cur_psnr, cur_ssim, msssimDB, bpp)))
        ...
        logger.info('{:20s} - sample {:04d} image: bpp = {:.4f}, psnr = {:.3f}dB, ssim = {:.4f}, ssim-dB = {:.4f}'.format(phase, i, bpp, cur_psnr, cur_ssim, msssimDB))

        test_metrics['psnr'].append(cur_psnr)
        test_metrics['ms-ssim'].append(cur_ssim)
        test_metrics['ms-ssim-dB'].append(msssimDB)
